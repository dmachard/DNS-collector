...
| [Unique Domain Responses](transformers/transform_uniqueresponsetracker.md) | • Track newly observed (QNAME, RRType, RDATA) tuples<br/>• Detect unprecedented DNS answer associations<br/>• Dual storage engines: LRU (default, ~192 ns/op) or Cuckoo Filter (optional, 81% memory reduction)<br/>• Disk persistence support (LRU engine) | • DNS hijacking & cache poisoning detection<br/>• Fast-Flux C2 tracking<br/>• Unauthorized zone record changes |
