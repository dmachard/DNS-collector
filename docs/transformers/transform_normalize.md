...
> **Note**: DNS-collector handles all strings (`qname`, `rdata`, etc.) as UTF-8. Any non-UTF-8 characters encountered during processing are replaced with the Unicode replacement character `` (`\uFFFD`). If you need to preserve the original non-UTF-8 bytes, use the **Data Extractor** transformer with `base64-fields` or `hex-fields`.
